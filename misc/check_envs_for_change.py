...
# ================================================================
ENVS = ["Ant-v0", "HalfCheetah-v0", "Hopper-v0", "Humanoid-v0",
        "InvertedDoublePendulum-v0", "Reacher-v0", "Swimmer-v0", "Walker2d-v0"]
OLD_COMMIT = "HEAD"


def cap(cmd):
    "Call and print command"
    print(gym.utils.colorize(cmd, "green"))
...
cap("rm -rf %(oldgymroot)s %(comparedir)s && mkdir %(comparedir)s && cd /tmp "
    "&& git clone %(gymroot)s %(oldgymbase)s" % locals())
for env in ENVS:
    print(gym.utils.colorize("*" * 50 + "\nENV: %s" % env, "red"))
    ...
    cap("python %(writescript)s %(env)s %(outfileB)s --gymdir=%(oldgymroot)s" %
        locals())
