...
def check_env(env: gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    ...
    # As environments are wrapped by OrderEnforcing generated by `gym.make`
    if has_wrapper(env, OrderEnforcing):
        try:
            env.step(env.action_space.sample())
        except ResetNeeded as e:
            assert str(e) == "Cannot call env.step() before calling env.reset()"
        try:
            env.render()
        except ResetNeeded as e:
            assert (
                str(e) == "Cannot call env.render() before calling env.reset(), "
                "if this is a intended action, set `disable_render_order_enforcing` "
                "on the OrderEnforcer wrapper = False."
            )
