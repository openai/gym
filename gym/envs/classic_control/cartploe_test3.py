# Original work by JackFurby https://github.com/JackFurby/CartPole-v0/blob/master/cartPole.py
# Edit by Xi Yu
import gym
import numpy as np
...
# Given a state of the environment, return its descreteState index in qTable
...
for run in range(RUNS):
	...
	while not done:
		...
		newQ = currentQ + LEARNING_RATE * (reward + DISCOUNT * maxFutureQ - currentQ)
...
# Double Q-learning
# Create bins and Q table
def create_bins_and_q_table_double():
	numBins = 20
	obsSpaceSizeD = len(env.observation_space.high)
	...
	return binsD, obsSpaceSizeD, qTableA, qTableB

# Given a state of the enviroment, return its descreteState index in qTable
def get_discrete_state_Double(state, binsD, obsSpaceSizeD):
	stateIndex = []
	for i in range(obsSpaceSize):
		stateIndex.append(np.digitize(state[i], bins[i]) - 1) # -1 will turn bin into index
	return tuple(stateIndex)

binsD, obsSpaceSizeD, qTableA, qTableB = create_bins_and_q_table_double()

previousCnt = []  # array of all scores over runs
# double Q-learning
run = 0
for run in range(RUNS):
	discreteState = get_discrete_state_Double(env.reset(), binsD, obsSpaceSizeD)
	done = False  # has the environment finished?
	cnt = 0  # how may movements cart has made
	...
	while not done:
		...
		# Get action from Q table
		if np.random.random() > epsilon:
			action = max(np.argmax(qTableA[discreteState]),np.argmax(qTableA[discreteState]))
		# Get random action
		else:
			action = np.random.randint(0, env.action_space.n)
		newState, reward, done, _ = env.step(action)  # perform action on enviroment

		newDiscreteState = get_discrete_state_Double(newState, binsD, obsSpaceSizeD)
  
		#Choose randomly either update qTableA or qTableB
		if (np.random.random()<0.5) :
			maxFutureQ = np.max(qTableA[newDiscreteState])  # estimate of optimal future value of qTableA
			currentQ = qTableA[discreteState + (action, )]  # old value

			# formula to caculate all Q values
			newQ = currentQ + LEARNING_RATE * (reward + DISCOUNT * maxFutureQ - currentQ)
			qTableA[discreteState + (action, )] = newQ  # Update qTable with new Q value
		else:
			maxFutureQ = np.max(qTableB[newDiscreteState])  # estimate of optimal future value of qTableB
			currentQ = qTableB[discreteState + (action, )]  # old value

			# formula to caculate all Q values
			newQ = currentQ + LEARNING_RATE * (reward + DISCOUNT * maxFutureQ - currentQ)
			qTableB[discreteState + (action, )] = newQ  # Update qTable with new Q value
