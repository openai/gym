...
class RobotTemperature(gym.Env):
    ...
    def __init__(self):
        ...
        # position, temp
        self.corridor_len = 10
        self.observation_space = spaces.MultiDiscrete([[1, self.corridor_len], [-20, 140]])
        self._seed()
        self.state = np.array([1, 50])
        self.viewer = None
    ...
    def _step(self, action):
        ...
        if self.state[0] < 3:
            corridor_temp = self.np_random.randint(5, 35)
        elif self.state[0] < 7:
            corridor_temp = self.np_random.randint(25, 75)
        else:
            corridor_temp = self.np_random.randint(65, 95)

        if temp_control == 0:  # cool
            self.state[1] = max(0, corridor_temp + self.np_random.randint(-45, -25))
        elif temp_control == 1:
            self.state[1] = corridor_temp
        elif temp_control == 2:
            self.state[1] = corridor_temp - self.np_random.randint(25, 45)
    ...
    def _reset(self):
        self.state[0] = self.observation_space.sample()[0]
        self.state[1] = 50
        return self.state
    ...
    def _render(self, mode='human', close=False):
        ...
        print(self.state)
        roboty = -robot_s / 2 + (self.state[0] * screen_height / self.corridor_len)
        self.robot_trans.set_translation(screen_width / 2, roboty)

        if self.state[1] > 50:
            r = (self.state[1] - 50) / 50.0
            b = 0
        else:
            r = 0
            b = self.state[1] / 50.0
