...
class SoccerEnv(gym.Env, utils.EzPickle):
    ...
    def _configure_environment(self):
      """Provides a chance for subclasses to override this method to supply
    ...
    def _start_hfo_server(self, headless=True, frames_per_trial=500,
                          untouched_time=100,
                          offense_agents=1, defense_agents=0,
                          offense_npcs=0, defense_npcs=0,
                          offense_team="base", defense_team="base",
                          no_sync=False, port=6000, record=False,
                          offense_on_ball=0, fullstate=False, seed=-1,
                          message_size=1000, ball_x_min=0.0,
                          ball_x_max=0.2, verbose=False, log_dir="log"):
    ...
    def _start_viewer(self):
      """ Start the SoccerWindow visualizer. Note the viewer may also be used
      with a rcg file to visualize a game. See details at
    ...
    def _get_reward(self):
      """ This sparse reward function only rewards the agent for scoring a goal. """
    ...
    def _reset(self):
      """ Repeats NO-OP until the episode terminates """
    ...
    def _render(self, mode='human', close=False):
      if close:
        if self.viewer is not None:
          os.kill(self.viewer_process.pid, signal.SIGKILL)
      else:
