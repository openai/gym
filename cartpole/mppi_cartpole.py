...
class MPPI():
    ...
    def _compute_cost_for_sample_n(self, n):
        ...
        for t in range(self.T):
            ...
            u_t = 0 if u_value <= 0.5 else 1
            ob, reward, done, info = self.env.mock_step(u_t)
            self.cost_for_N_sample[n] += -reward # such that minimizing cost is same as maximizing reward
            if done:
                break
    ...
    def roll_out(self, iteration):
        ...
        for iter in range(iteration):
            ...
            for i in range(TIMESTEPS):
                self.U[i] = 0 if temp_U[i] <= 0.5 else 1
...
if __name__ == "__main__":
    ...
    ACTION_MAX = 1 # push cart to the right
    ...
    logging.basicConfig(filename="cartpole_mppi_action_cost.log", filemode='w', format='%(levelname)s - %(message)s', level=logging.CRITICAL)
