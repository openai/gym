...
class TabularQAgent(object):
    ...
    def __init__(self, observation_space, action_space, **userconfig):
        ...
        self.config = {
            ...
            "learning_rate" : 0.1,  #(rate 0.1 to 0.5 for roulette)
            "eps": 0.05,           # Epsilon in epsilon greedy policies (from 0.05 to 0.01 for roulette)
            "discount": 0.99,#from 0.95 to 0.99 for roulette
    ...
    def act(self, observation, eps=None):
        ...
        if np.random.random() > 1-eps:
            action = np.argmax(self.q[observation.item()])
        else: 
