...
def test_step_timeout_async_vector_env(shared_memory):
    ...
    with pytest.raises(TimeoutError):
        try:
            ...
            observations, rewards, terminateds, truncateds, _ = env.step_wait(
                timeout=0.1
            )
        finally:
            env.close(terminate=True)
...
def test_step_out_of_order_async_vector_env(shared_memory):
    ...
    with pytest.raises(NoAsyncCallError):
        try:
            ...
            actions = env.action_space.sample()
            observations = env.reset()
            observations, rewards, terminateds, truncateds, infos = env.step_wait()
...
def test_custom_space_async_vector_env():
    ...
    try:
        ...
        step_observations, rewards, termianteds, truncateds, _ = env.step(actions)
    finally:
        env.close()
