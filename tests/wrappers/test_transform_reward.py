...
def test_transform_reward(env_id):
    ...
    for scale in scales:
        ...
        wrapped_env = TransformReward(
            gym.make(env_id, return_two_dones=True), lambda r: scale * r
        )
    ...
    env = gym.make(env_id, return_two_dones=True)
    wrapped_env = TransformReward(
        gym.make(env_id, return_two_dones=True), lambda r: np.clip(r, min_r, max_r)
    )
    action = env.action_space.sample()
    ...
    env = gym.make(env_id, return_two_dones=True)
    wrapped_env = TransformReward(
        gym.make(env_id, return_two_dones=True), lambda r: np.sign(r)
    )
